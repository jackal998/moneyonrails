class SubAccount < ApplicationRecord
  ...
  def display_key
    raw_key = crypt.decrypt_and_verify(self[:encrypted_public_key])
  ...
  def ftx_api_validation?
    ...
    error_msg_filter(ftx_errors)["error"].empty?
  end
  ...
  def error_msg_filter(ftx_errors)
    ...
    return {"error" => "API have no permissions for subaccount #{name}"} if ftx_errors["cancel"][0..35] == "Only have permissions for subaccount"
