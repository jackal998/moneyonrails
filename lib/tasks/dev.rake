...
namespace :dev do
  ...
  task update_funding_payment: :environment do
    ...
    funding_orders.each do |coin_name, funding_order|
      if recent_orders[coin_name]
        if recent_orders[coin_name].first[:original_perp_amount] == 0 && recent_orders[coin_name].last[:target_perp_amount] != 0
          updated_status[coin_name] = "new"
          orders_to_be_updated += 1
        elsif recent_orders[coin_name].first[:original_perp_amount] != 0 && recent_orders[coin_name].last[:target_perp_amount] != 0
          updated_status[coin_name] = "normal"
          orders_to_be_updated += 1
        elsif recent_orders[coin_name].last[:target_perp_amount] == 0
          updated_status[coin_name] = "down"
        end
      elsif (funding_order[:order_status] == "Close" && funding_order[:target_perp_amount] != 0 && funding_order[:updated_at] <= query_end_time) ||
      ...
      elsif funding_order[:order_status] == "Underway"
    ...
    data_results = data["result"].index_by { |result| (result["future"].split("-")[0]).to_s }
    ...
    updated_status.each do |coin_name, status|
      ...
      when "normal", "underway", "new"
        if data_results[coin_name]
          ...
          puts "#{coin_name} order #{funding_orders[coin_name][:id]} status normal, but no data from FTX"
        end
  ...
  task update_rate: :environment do
    ...
    rate_datas_tbu = []
    tmp = data["result"].index_by { |result| (result["future"].split("-")[0]).to_s }
    ...
    puts 'tmp.except!("DMG")'
    err_msg = if datas_count == coins_to_update && last_2hr_rates_count == coins_to_update * 2
      "Rates already up-to-date, no data was imported."
    elsif datas_count > coins_to_update
      "Please update coins list first  => rails c > `helper.update_market_infos`
               \rthen update history rates data  => `rake dev:fetch_history_rate`"
    else
      "Update history rates data first => `rake dev:fetch_history_rate`"
    end
  ...
  task fetch_history_rate: :environment do
    ...
    coins.each do |c|
      ...
      lastest_data_time = if c.rates.empty?
        "2019-03-01T00:00:00+00:00".to_time
      else
        c.rates.order("time").last.time
      end
  ...
  def calc_rate_stats(c)
    ...
    crs = c.current_fund_stat || CurrentFundStat.new(coin: c, created_at: Time.now, updated_at: Time.now)
